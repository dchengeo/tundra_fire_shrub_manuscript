pro ABOVE_tundra_fire_drop_no_vegetation_type_sample_points_V14c
...
outfile='C:\Users\itscd\Dropbox\Tables\ABoVE_Tundra_Fire\extracted_max_NDVI_merged_with_fireyear_severity_V14c_refined.csv'
...
pidlist_veg=indata_csv.field03
vegvalue_list=indata_csv.field10

outdata=fltarr(8, 1)
outdata_preserved=lonarr(1, 1)

for PID=100000, 119999 do begin
    if vegvalue_list[where(pidlist_veg eq PID)] eq -9999 then continue
    ...
    if total([watervalue1, watervalue2, watervalue3]) eq 0 then begin
        ...
        temp[7, *]=vegvalue_list[where(pidlist_veg eq PID)]
...
write_csv, outfile, outdata, header=[header, 'VegType']
